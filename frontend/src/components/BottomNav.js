...
export default function BottomNav({ active = "home" }) {
  ...
  const tabClass = (name) =>
    `flex flex-col items-center gap-1 ${
      active === name ? "text-primary" : "text-slate-400 dark:text-white/40"
    }`;

  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50">
      <div className="max-w-[430px] mx-auto bg-white/90 dark:bg-[#10221c]/90 backdrop-blur-xl border-t border-slate-200 dark:border-white/10 px-6 pb-8 pt-3 flex justify-between items-center relative">
